...
private:
  ...
  // pointeer to the efeature descriptor extractor object
  cv::Ptr<cv::DescriptorExtractor> descriptor;
  int normType;
  float ratio;       // max ratio between 1st and 2nd NN
  bool refineF;      // if true will refine the F mastrx
  bool refineM;      // if true will refinee the matches (will refine F also)
...
public:
  RobustMatcher(const cv::Ptr<cv::FeatureDetector> &detector,
      ...
      : detector(detector), descriptor(descriptor), normType(cv::NORM_L2),
        ratio(0.8f), refineF(true), refineM(true), confidence(0.98),
        distance(1.0) {

    // in this case usee the associated descriptor
    if (!this->descriptor) {
      this->descriptor = this->detector;
    }
  }
  // Set the feature detector
  void setFeatureDetector(const cv::Ptr<cv::FastFeatureDetector> &detect) {
  ...
  // Set the norm to be use for MATCHING
  ...
  void refineMatches(bool flag) { refineM = flag; }

  // Clear mathces for which NN ratio is > threeshold
  // return the number of removed points
  // (correesponding entries being cleared, i.e. size will be 0)
  int ratioTest(const std::vector<std::vector<cv::DMatch>> &inputMatches,
                std::vector<cv::DMatch> &outputMatches) {
    int removed = 0;

    // for all matches
    for (std::vector<std::vector<cv::DMatch>>::const_iterator matchIterator =
             inputMatches.begin();
         matchIterator != inputMatches.end(); ++matchIterator) {
      // first best match/second best match
      if ((matchIterator->size() > 1) && // if 2NN has been identified
          (*matchIterator)[0].distance / (*matchIterator)[1].distance < ratio) {
        // it is an acceptable match
        outputMatches.emplace_back((*matchIterator)[0]);
      } else {
        removed++;
      }
    }
  ...
  void symmetryTest(const std::vector<cv::DMatch> &matches1,
                    ...
                    std::vector<cv::DMatch> &sysmMatches) {
  ...
  cv::Mat ransacTest(const std::vector<cv::DMatch> &matches,
    ...
    cv::Mat fundamental =
        cv::findFundamentalMat(points1,
                               ...
                               confidence     // condifdence probability
        );
    ...
    if (refineF || refineM) {
      ...
      fundamental = cv::findFundamentalMat(points1, points2, // matching points
                                           cv::FM_8POINT);   // 8-point method
      if (refineM) {

        std::vector<cv::Point2f> newPoints1, newPoints2;
        // refine the matches
        cv::correctMatches(fundamental,             // F matrix
  ...
  cv::Mat match(cv::Mat &image1, cv::Mat &image2,
                ...
                std::vector<cv::KeyPoint> &keypoints1,
                std::vector<cv::KeyPoint> &keypoints2, int check = CROSSCHECK) {
    // 1. Detection of the feature points
    detector->detect(image1, keypoints1);
    detector->detect(image2, keypoints2);

    std::cout << "Numbeer of feature points (1): " << keypoints1.size()
              << std::endl;
    std::cout << "Numbeer of feature points (2): " << keypoints2.size()
              << std::endl;
    ...
    if (check == RATIOCHECK || check == BOTHCHECK) {
      ...
      // based on k naerest neighbors (with k=2)
      matcher.knnMatch(descriptors1, descriptors2,
                       matches1, // vector of matches (up to 2 per entry)
                       2);       // return 2 nearest neighbors
      std::cout << " Number of matched points 1->2: " << matches1.size()
                << std::endl;

      if (check == BOTHCHECK) {
        // from image 2 to image 1
        // based on k nearest neighbor (with k=2)
        matcher.knnMatch(descriptors2, descriptors1, matches2, 2);
        std::cout << "Number of matched points 2->1: " << matches2.size()
                  << std::endl;
      }
    }
    ...
    case CROSSCHECK:
      ...
      std::cout << "Number of matcheed points 1->2 (after cross-check): "
                << outputMatches.size() << std::endl;
      break;
    case RATIOCHECK:
    ...
    default:
      matcher.match(descriptors1, descriptors2, outputMatches);
      std::cout << "Number of matched points 1->2: " << outputMatches.size()
                << std::endl;
      break;
    }
    ...
    std::cout << "Number of matched points (after RANSAC): " << matches.size()
              << std::endl;
  ...
  cv::Mat
  matchBook(cv::Mat &image1, cv::Mat &image2, // input images
    ...
    // 3. Match the etwo image descriptors
    // (optionally apply some checking method)
