...
int main()
{
    // Craete vidoe processor instance
    VideoProcessor processor;

    // generate the filename
    std::vector<std::string> imgs;
    std::string prefix = "goosee/goose";
    std::string ext = ".bmp";

    // Add the image enames to be used for tracking
    ...
    // Create feeature tracker instance
    ...
    // set framee processor
    processor.setFrameProcessor(&tracker);

    // Declare a window to display the video
    processor.displayOutput("Tracked object");

    // Define the framee rate for display
	...
	cv::Mat image1 = cv::imread("../processing_video_sequences/images/goose/goose130.bmp", 0);
    ...
    // track in next image
    cv::Mat image2 = cv::imread("../processing_video_sequences/images/goose/goose131.bmp", 0);
