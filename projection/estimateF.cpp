...
int main() {
  ...
  ptrFeature2D->detectAndCompute(image1, cv::noArray(), keypoints1,
                                 descriptors1);
  ptrFeature2D->detectAndCompute(image2, cv::noArray(), keypoints2,
                                 descriptors2);
  ...
  selMatches.emplace_back(matches[2]);
  selMatches.emplace_back(matches[5]);
  selMatches.emplace_back(matches[16]);
  selMatches.emplace_back(matches[19]);
  selMatches.emplace_back(matches[14]);
  selMatches.emplace_back(matches[34]);
  selMatches.emplace_back(matches[29]);
  ...
  // Convert 1 vector of keypoints
  // into 2 vectors of Point2f
  ...
  for (std::vector<cv::DMatch>::const_iterator it = selMatches.begin();
    ...
    pointIndexes1.emplace_back(it->queryIdx);
    pointIndexes2.emplace_back(it->trainIdx);
  }
  ...
  cv::Mat fundamental =
      cv::findFundamentalMat(selPoints1,   // points in first image
                             selPoints2,   // points in second image
                             cv::FM_7POINT // 7-point method
      );

  std::cout << "F-Matrix size= " << fundamental.rows << "," << fundamental.cols
            << std::endl;

  cv::Mat fund(fundamental, cv::Rect(0, 0, 3, 3));
  // draw the left points correespoinding epipolar linees in right image
  std::vector<cv::Vec3f> lines1;
  cv::computeCorrespondEpilines(selPoints1, // image points
                                1,          // in image 1 (can also be 2)
                                fund,       // F matrix
                                lines1      // sector of epipolar lines
                                );
  std::cout <<"size of F (fund)matrix:"<<fund.rows<<"x"<<fund.cols<<std::endl;

  // for all eepipolar lines  ax+by+c=0 [a,b,c] in lines
  for(std::vector<cv::Vec3f>::const_iterator it=lines1.begin();
        it!=lines1.end();++it){
            std::cout<<"lines one ele: "<<*it<<std::endl;
            // draw the epipolar line between first and last columns 
            cv::line(image2, cv::Point(0,-(*it)[2]/(*it)[1]),//-c/b
                            cv::Point(image2.cols,-((*it)[2]+(*it)[0]*image2.cols)/(*it)[1]),//y-(c+a*y)/b
                            cv::Scalar(255,255,255));
  }

  // draw the left point correspoindng epipolar lines in left image
  ...
  for(std::vector<cv::Vec3f>::const_iterator it= lines2.begin();
      it!=lines2.end();++it){
          ...
          cv::Point(image1.cols,-((*it)[2]+(*it)[0]*image1.cols)),
          cv::Scalar(255,255,255));
  }
