...
class FeatureTracker : public FrameProcessor{
	...
	void process(cv:: Mat &frame, cv:: Mat &output) {
        ...
        // 3. loopo over the tracked points to reject the undesirables
        int k=0;
        for (int i=0; i < points[1].size();i++){
            // do we keep this points?
    ...
    void detectFeaturePoints(){
        ...
        minDist // min distance betweeen two features
        );
    }

    // determin if new points should be added
    bool addNewPoints(){
    ...
    // determin which tracked point should be accepted
    // here we keep only monving points
